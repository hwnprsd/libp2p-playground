...
type SCInterface interface {
	GetSquadID(ID peer.ID) string
	GetPeerList(squadID string) []peer.ID
	GetWalletsUnderManagement(squadID string) []WalletData
}

type TestContract struct{}

func (*TestContract) GetSquadID(ID peer.ID) string {
	return "SQUAD_1"
}

func (*TestContract) GetPeerList(squadID string) []peer.ID {
...
func (*TestContract) GetWalletsUnderManagement(squadID string) []WalletData {
